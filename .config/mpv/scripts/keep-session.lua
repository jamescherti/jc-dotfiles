-- Based on: https://raw.githubusercontent.com/CogentRedTester/mpv-scripts/master/keep-session.lua
--[[
...
local function save_playlist(file)
    ...
    if not session then
        return msg.error("Failed to write to file", file)
    end
...
local function load_prev_session(file)
    ...
    -- TODO crashes when all the files that are in the list that is loaded do not exist anymore
    -- mp.commandv("loadfile", "test", "append")
...
local interval_save_playlist = 60.0  -- seconds
mp.add_periodic_timer(interval_save_playlist, function()
    if o.auto_save then
